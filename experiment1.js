...
input.on('message', (deltaTime, message) => {
  ...
  switch(message[0]) {
    case 0xf8: // timing clock sent every 1/24th of quarter note
      //console.log('.')
      if (readyForNext && ticks == 24*4-1 && playing) {
        readyForNext = false;
        currentProgram++;
        currentProgram = currentProgram % 8;
        output.sendMessage([0xc9,currentProgram]);
        console.log(`sent PC 0xc9, ${currentProgram}`);
      }
      
      ticks++
      if (ticks == 24 * 4) {
        console.log('24x4 clocks = whole note')
        ticks = 0
      }
      break;
    case 0xfa:
    ...
    case 0xc9: // program change received
      readyForNext = true; // pc received, queue up next one
    default:
